"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.msgTypes = void 0;
const tx_1 = require("./types/ibc/core/channel/v1/tx");
const upgrade_1 = require("./types/ibc/core/channel/v1/upgrade");
const query_1 = require("./types/ibc/core/channel/v1/query");
const tx_2 = require("./types/ibc/core/channel/v1/tx");
const tx_3 = require("./types/ibc/core/channel/v1/tx");
const query_2 = require("./types/ibc/core/channel/v1/query");
const query_3 = require("./types/ibc/core/channel/v1/query");
const query_4 = require("./types/ibc/core/channel/v1/query");
const query_5 = require("./types/ibc/core/channel/v1/query");
const query_6 = require("./types/ibc/core/channel/v1/query");
const channel_1 = require("./types/ibc/core/channel/v1/channel");
const tx_4 = require("./types/ibc/core/channel/v1/tx");
const query_7 = require("./types/ibc/core/channel/v1/query");
const query_8 = require("./types/ibc/core/channel/v1/query");
const tx_5 = require("./types/ibc/core/channel/v1/tx");
const query_9 = require("./types/ibc/core/channel/v1/query");
const query_10 = require("./types/ibc/core/channel/v1/query");
const query_11 = require("./types/ibc/core/channel/v1/query");
const channel_2 = require("./types/ibc/core/channel/v1/channel");
const tx_6 = require("./types/ibc/core/channel/v1/tx");
const tx_7 = require("./types/ibc/core/channel/v1/tx");
const tx_8 = require("./types/ibc/core/channel/v1/tx");
const query_12 = require("./types/ibc/core/channel/v1/query");
const tx_9 = require("./types/ibc/core/channel/v1/tx");
const tx_10 = require("./types/ibc/core/channel/v1/tx");
const tx_11 = require("./types/ibc/core/channel/v1/tx");
const channel_3 = require("./types/ibc/core/channel/v1/channel");
const genesis_1 = require("./types/ibc/core/channel/v1/genesis");
const tx_12 = require("./types/ibc/core/channel/v1/tx");
const query_13 = require("./types/ibc/core/channel/v1/query");
const query_14 = require("./types/ibc/core/channel/v1/query");
const tx_13 = require("./types/ibc/core/channel/v1/tx");
const tx_14 = require("./types/ibc/core/channel/v1/tx");
const query_15 = require("./types/ibc/core/channel/v1/query");
const query_16 = require("./types/ibc/core/channel/v1/query");
const channel_4 = require("./types/ibc/core/channel/v1/channel");
const genesis_2 = require("./types/ibc/core/channel/v1/genesis");
const tx_15 = require("./types/ibc/core/channel/v1/tx");
const tx_16 = require("./types/ibc/core/channel/v1/tx");
const tx_17 = require("./types/ibc/core/channel/v1/tx");
const query_17 = require("./types/ibc/core/channel/v1/query");
const query_18 = require("./types/ibc/core/channel/v1/query");
const query_19 = require("./types/ibc/core/channel/v1/query");
const channel_5 = require("./types/ibc/core/channel/v1/channel");
const channel_6 = require("./types/ibc/core/channel/v1/channel");
const tx_18 = require("./types/ibc/core/channel/v1/tx");
const tx_19 = require("./types/ibc/core/channel/v1/tx");
const tx_20 = require("./types/ibc/core/channel/v1/tx");
const query_20 = require("./types/ibc/core/channel/v1/query");
const query_21 = require("./types/ibc/core/channel/v1/query");
const query_22 = require("./types/ibc/core/channel/v1/query");
const tx_21 = require("./types/ibc/core/channel/v1/tx");
const tx_22 = require("./types/ibc/core/channel/v1/tx");
const tx_23 = require("./types/ibc/core/channel/v1/tx");
const tx_24 = require("./types/ibc/core/channel/v1/tx");
const upgrade_2 = require("./types/ibc/core/channel/v1/upgrade");
const tx_25 = require("./types/ibc/core/channel/v1/tx");
const query_23 = require("./types/ibc/core/channel/v1/query");
const query_24 = require("./types/ibc/core/channel/v1/query");
const query_25 = require("./types/ibc/core/channel/v1/query");
const tx_26 = require("./types/ibc/core/channel/v1/tx");
const upgrade_3 = require("./types/ibc/core/channel/v1/upgrade");
const tx_27 = require("./types/ibc/core/channel/v1/tx");
const query_26 = require("./types/ibc/core/channel/v1/query");
const tx_28 = require("./types/ibc/core/channel/v1/tx");
const query_27 = require("./types/ibc/core/channel/v1/query");
const query_28 = require("./types/ibc/core/channel/v1/query");
const channel_7 = require("./types/ibc/core/channel/v1/channel");
const channel_8 = require("./types/ibc/core/channel/v1/channel");
const tx_29 = require("./types/ibc/core/channel/v1/tx");
const tx_30 = require("./types/ibc/core/channel/v1/tx");
const query_29 = require("./types/ibc/core/channel/v1/query");
const tx_31 = require("./types/ibc/core/channel/v1/tx");
const tx_32 = require("./types/ibc/core/channel/v1/tx");
const tx_33 = require("./types/ibc/core/channel/v1/tx");
const query_30 = require("./types/ibc/core/channel/v1/query");
const tx_34 = require("./types/ibc/core/channel/v1/tx");
const query_31 = require("./types/ibc/core/channel/v1/query");
const query_32 = require("./types/ibc/core/channel/v1/query");
const channel_9 = require("./types/ibc/core/channel/v1/channel");
const tx_35 = require("./types/ibc/core/channel/v1/tx");
const tx_36 = require("./types/ibc/core/channel/v1/tx");
const tx_37 = require("./types/ibc/core/channel/v1/tx");
const tx_38 = require("./types/ibc/core/channel/v1/tx");
const query_33 = require("./types/ibc/core/channel/v1/query");
const query_34 = require("./types/ibc/core/channel/v1/query");
const msgTypes = [
    ["/ibc.core.channel.v1.MsgTimeoutOnClose", tx_1.MsgTimeoutOnClose],
    ["/ibc.core.channel.v1.ErrorReceipt", upgrade_1.ErrorReceipt],
    ["/ibc.core.channel.v1.QueryUpgradeErrorRequest", query_1.QueryUpgradeErrorRequest],
    ["/ibc.core.channel.v1.MsgChannelCloseInit", tx_2.MsgChannelCloseInit],
    ["/ibc.core.channel.v1.MsgTimeout", tx_3.MsgTimeout],
    ["/ibc.core.channel.v1.QueryPacketCommitmentRequest", query_2.QueryPacketCommitmentRequest],
    ["/ibc.core.channel.v1.QueryNextSequenceReceiveResponse", query_3.QueryNextSequenceReceiveResponse],
    ["/ibc.core.channel.v1.QueryConnectionChannelsRequest", query_4.QueryConnectionChannelsRequest],
    ["/ibc.core.channel.v1.QueryPacketReceiptRequest", query_5.QueryPacketReceiptRequest],
    ["/ibc.core.channel.v1.QueryNextSequenceSendRequest", query_6.QueryNextSequenceSendRequest],
    ["/ibc.core.channel.v1.Counterparty", channel_1.Counterparty],
    ["/ibc.core.channel.v1.MsgChannelUpgradeOpenResponse", tx_4.MsgChannelUpgradeOpenResponse],
    ["/ibc.core.channel.v1.QueryChannelClientStateResponse", query_7.QueryChannelClientStateResponse],
    ["/ibc.core.channel.v1.QueryChannelsResponse", query_8.QueryChannelsResponse],
    ["/ibc.core.channel.v1.MsgTimeoutOnCloseResponse", tx_5.MsgTimeoutOnCloseResponse],
    ["/ibc.core.channel.v1.QueryChannelConsensusStateResponse", query_9.QueryChannelConsensusStateResponse],
    ["/ibc.core.channel.v1.QueryChannelResponse", query_10.QueryChannelResponse],
    ["/ibc.core.channel.v1.QueryPacketAcknowledgementResponse", query_11.QueryPacketAcknowledgementResponse],
    ["/ibc.core.channel.v1.Acknowledgement", channel_2.Acknowledgement],
    ["/ibc.core.channel.v1.MsgChannelOpenTry", tx_6.MsgChannelOpenTry],
    ["/ibc.core.channel.v1.MsgChannelUpgradeTry", tx_7.MsgChannelUpgradeTry],
    ["/ibc.core.channel.v1.MsgChannelUpgradeAck", tx_8.MsgChannelUpgradeAck],
    ["/ibc.core.channel.v1.QueryChannelParamsResponse", query_12.QueryChannelParamsResponse],
    ["/ibc.core.channel.v1.MsgAcknowledgement", tx_9.MsgAcknowledgement],
    ["/ibc.core.channel.v1.MsgChannelUpgradeTimeout", tx_10.MsgChannelUpgradeTimeout],
    ["/ibc.core.channel.v1.MsgChannelUpgradeTimeoutResponse", tx_11.MsgChannelUpgradeTimeoutResponse],
    ["/ibc.core.channel.v1.PacketState", channel_3.PacketState],
    ["/ibc.core.channel.v1.GenesisState", genesis_1.GenesisState],
    ["/ibc.core.channel.v1.MsgChannelOpenAck", tx_12.MsgChannelOpenAck],
    ["/ibc.core.channel.v1.QueryPacketCommitmentsRequest", query_13.QueryPacketCommitmentsRequest],
    ["/ibc.core.channel.v1.QueryNextSequenceReceiveRequest", query_14.QueryNextSequenceReceiveRequest],
    ["/ibc.core.channel.v1.MsgChannelOpenTryResponse", tx_13.MsgChannelOpenTryResponse],
    ["/ibc.core.channel.v1.MsgChannelUpgradeTryResponse", tx_14.MsgChannelUpgradeTryResponse],
    ["/ibc.core.channel.v1.QueryPacketAcknowledgementRequest", query_15.QueryPacketAcknowledgementRequest],
    ["/ibc.core.channel.v1.QueryUnreceivedAcksResponse", query_16.QueryUnreceivedAcksResponse],
    ["/ibc.core.channel.v1.Packet", channel_4.Packet],
    ["/ibc.core.channel.v1.PacketSequence", genesis_2.PacketSequence],
    ["/ibc.core.channel.v1.MsgUpdateParams", tx_15.MsgUpdateParams],
    ["/ibc.core.channel.v1.MsgChannelOpenConfirm", tx_16.MsgChannelOpenConfirm],
    ["/ibc.core.channel.v1.MsgChannelOpenConfirmResponse", tx_17.MsgChannelOpenConfirmResponse],
    ["/ibc.core.channel.v1.QueryConnectionChannelsResponse", query_17.QueryConnectionChannelsResponse],
    ["/ibc.core.channel.v1.QueryUnreceivedAcksRequest", query_18.QueryUnreceivedAcksRequest],
    ["/ibc.core.channel.v1.QueryNextSequenceSendResponse", query_19.QueryNextSequenceSendResponse],
    ["/ibc.core.channel.v1.PacketId", channel_5.PacketId],
    ["/ibc.core.channel.v1.Params", channel_6.Params],
    ["/ibc.core.channel.v1.MsgChannelOpenInit", tx_18.MsgChannelOpenInit],
    ["/ibc.core.channel.v1.MsgChannelUpgradeConfirm", tx_19.MsgChannelUpgradeConfirm],
    ["/ibc.core.channel.v1.MsgChannelUpgradeCancelResponse", tx_20.MsgChannelUpgradeCancelResponse],
    ["/ibc.core.channel.v1.QueryPacketAcknowledgementsRequest", query_20.QueryPacketAcknowledgementsRequest],
    ["/ibc.core.channel.v1.QueryUnreceivedPacketsResponse", query_21.QueryUnreceivedPacketsResponse],
    ["/ibc.core.channel.v1.QueryUpgradeErrorResponse", query_22.QueryUpgradeErrorResponse],
    ["/ibc.core.channel.v1.MsgChannelUpgradeInit", tx_21.MsgChannelUpgradeInit],
    ["/ibc.core.channel.v1.MsgChannelUpgradeCancel", tx_22.MsgChannelUpgradeCancel],
    ["/ibc.core.channel.v1.MsgChannelCloseInitResponse", tx_23.MsgChannelCloseInitResponse],
    ["/ibc.core.channel.v1.MsgChannelCloseConfirmResponse", tx_24.MsgChannelCloseConfirmResponse],
    ["/ibc.core.channel.v1.UpgradeFields", upgrade_2.UpgradeFields],
    ["/ibc.core.channel.v1.MsgPruneAcknowledgementsResponse", tx_25.MsgPruneAcknowledgementsResponse],
    ["/ibc.core.channel.v1.QueryPacketCommitmentsResponse", query_23.QueryPacketCommitmentsResponse],
    ["/ibc.core.channel.v1.QueryUpgradeRequest", query_24.QueryUpgradeRequest],
    ["/ibc.core.channel.v1.QueryChannelParamsRequest", query_25.QueryChannelParamsRequest],
    ["/ibc.core.channel.v1.MsgChannelCloseConfirm", tx_26.MsgChannelCloseConfirm],
    ["/ibc.core.channel.v1.Upgrade", upgrade_3.Upgrade],
    ["/ibc.core.channel.v1.MsgChannelUpgradeInitResponse", tx_27.MsgChannelUpgradeInitResponse],
    ["/ibc.core.channel.v1.QueryChannelClientStateRequest", query_26.QueryChannelClientStateRequest],
    ["/ibc.core.channel.v1.MsgRecvPacketResponse", tx_28.MsgRecvPacketResponse],
    ["/ibc.core.channel.v1.QueryChannelConsensusStateRequest", query_27.QueryChannelConsensusStateRequest],
    ["/ibc.core.channel.v1.QueryUpgradeResponse", query_28.QueryUpgradeResponse],
    ["/ibc.core.channel.v1.IdentifiedChannel", channel_7.IdentifiedChannel],
    ["/ibc.core.channel.v1.Timeout", channel_8.Timeout],
    ["/ibc.core.channel.v1.MsgChannelUpgradeOpen", tx_29.MsgChannelUpgradeOpen],
    ["/ibc.core.channel.v1.MsgChannelOpenInitResponse", tx_30.MsgChannelOpenInitResponse],
    ["/ibc.core.channel.v1.QueryUnreceivedPacketsRequest", query_29.QueryUnreceivedPacketsRequest],
    ["/ibc.core.channel.v1.MsgTimeoutResponse", tx_31.MsgTimeoutResponse],
    ["/ibc.core.channel.v1.MsgAcknowledgementResponse", tx_32.MsgAcknowledgementResponse],
    ["/ibc.core.channel.v1.MsgChannelUpgradeConfirmResponse", tx_33.MsgChannelUpgradeConfirmResponse],
    ["/ibc.core.channel.v1.QueryPacketAcknowledgementsResponse", query_30.QueryPacketAcknowledgementsResponse],
    ["/ibc.core.channel.v1.MsgRecvPacket", tx_34.MsgRecvPacket],
    ["/ibc.core.channel.v1.QueryChannelsRequest", query_31.QueryChannelsRequest],
    ["/ibc.core.channel.v1.QueryPacketCommitmentResponse", query_32.QueryPacketCommitmentResponse],
    ["/ibc.core.channel.v1.Channel", channel_9.Channel],
    ["/ibc.core.channel.v1.MsgPruneAcknowledgements", tx_35.MsgPruneAcknowledgements],
    ["/ibc.core.channel.v1.MsgChannelUpgradeAckResponse", tx_36.MsgChannelUpgradeAckResponse],
    ["/ibc.core.channel.v1.MsgChannelOpenAckResponse", tx_37.MsgChannelOpenAckResponse],
    ["/ibc.core.channel.v1.MsgUpdateParamsResponse", tx_38.MsgUpdateParamsResponse],
    ["/ibc.core.channel.v1.QueryChannelRequest", query_33.QueryChannelRequest],
    ["/ibc.core.channel.v1.QueryPacketReceiptResponse", query_34.QueryPacketReceiptResponse],
];
exports.msgTypes = msgTypes;
