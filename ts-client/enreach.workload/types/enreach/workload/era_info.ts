...
function createBaseEraInfo(): EraInfo {
  return { number: 0, startTime: undefined, startBlock: 0, endTime: undefined, endBlock: 0 };
}

export const EraInfo = {
  encode(message: EraInfo, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.number !== 0) {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): EraInfo {
    ...
    const message = createBaseEraInfo();
  ...
  create<I extends Exact<DeepPartial<EraInfo>, I>>(base?: I): EraInfo {
    return EraInfo.fromPartial(base ?? ({} as any));
  },
